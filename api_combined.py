#!/usr/bin/env python3
"""
Combined Flask API server for Stop Super Speeders.
Includes both heatmap endpoints and camera/ISA enforcement endpoints.

Usage:
    python api_combined.py
...
# Pre-computed detections for each camera
CAMERA_DETECTIONS = {
    "CAM-1": [
...
# =============================================================================
# CAMERA & ISA ENFORCEMENT ENDPOINTS
# =============================================================================
...
def simulate_detection(camera_id):
    ...
    try:
            ...
            if driver_row:
                ...
                if risk_pts >= ISA_REQUIRED_THRESHOLD and isa_status != "ISA_REQUIRED":
                    ...
                    alerts_created.append({
                        "alert_id": alert_id, "plate_id": plate_id,
                        "alert_type": "ISA_REQUIRED", "risk_score": risk_pts
                    })
                ...
                drivers_updated.append({
                    "plate_id": plate_id, "total_violations": total_v,
                    "risk_points": risk_pts, "isa_status": new_status,
                    "isa_required": new_status == "ISA_REQUIRED"
                })
            
            results.append({
                "violation_id": violation_id, "plate_id": plate_id,
                "violation_type": det["violation_type"], "points": det["points"],
                "speed_detected": det.get("speed_detected"), "speed_limit": det.get("speed_limit"),
        ...
        return jsonify({
            "camera_id": camera_id, "violations_logged": len(results),
            "detections": results, "drivers_updated": drivers_updated,
...
def reset_demo():
    ...
    try:
        ...
        cur.execute("""
            UPDATE drivers SET 
                total_violations = 0, current_risk_points = 0, isa_status = 'NONE',
                first_violation_date = NULL, last_violation_date = NULL
...
if __name__ == '__main__':
    print("=" * 60)
    print("  Stop Super Speeders - Combined API Server")
    print("=" * 60)
    print("\n  Heatmap Endpoints:")
    print("    GET  /api/stats")
    print("    GET  /api/heatmap")
    print("\n  Camera & ISA Endpoints:")
